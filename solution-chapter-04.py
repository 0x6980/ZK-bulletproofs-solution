...
def commit(a, sL, b, sR, alpha, beta, gamma, tau_1, tau_2):
    ...
    V = add(multiply(G, (a * b) % p), multiply(B, gamma))
    T1 = add(multiply(G, (a * sR + b * sL) % p), multiply(B, tau_1))
    T2 = add(multiply(G, (sR * sL) % p), multiply(B, tau_2))
    return (A, S, V, T1, T2)
